...
class Spi2Parallel {
public:
    Spi2Parallel(uint8_t pinCS, uint8_t devicesCount, uint8_t bitOrder = MSBFIRST);
    Spi2Parallel(uint8_t pinCS, uint8_t pinMOSI, uint8_t pinSCK, uint8_t devicesCount, uint8_t bitOrder = MSBFIRST);
    ~Spi2Parallel();
    void begin();
    void writeBit(uint8_t bit, bool value, uint8_t device);
    void writeByte(uint8_t value, uint8_t device);
    bool readBit(uint8_t bit, uint8_t device);
    uint8_t readByte(uint8_t device);
    void update();
